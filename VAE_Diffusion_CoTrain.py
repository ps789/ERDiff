...
for epoch in tqdm_notebook(range(n_epochs)):
    ...
    with torch.no_grad():
        ...
        if val_total_loss < pre_total_loss_: 
            ...
            torch.save(model.state_dict(),'model_checkpoints/source_vae_model_conditional' if conditional else 'model_checkpoints/source_vae_model')


            np.save("./npy_files/train_latents_conditional.npy" if conditional else "./npy_files/train_latents.npy",train_latents)
    ...
    for step, batch in enumerate(dataloader):
        ...
        batch[0], batch[1] = batch[0].to(device), batch[1].to(device).long()
...
np.save("conditional_losses.npy" if conditional else "losses.npy", loss_array)
